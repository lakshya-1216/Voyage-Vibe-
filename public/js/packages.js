...
document.querySelector('form').addEventListener('submit', function (event) {
    ...
    var place = document.querySelector('#place').value; // Assuming "place" is the select element ID

    if (place === "select-Value") {
        alert("Please select a destination!");
    } else if (place === "paris") {
        window.location.href = "/paris"; // Redirect to /paris
    } else if (place === "new_york") {
        window.location.href = "/newyork"; // Redirect to /newyork
    } else if (place === "tokyo") {
        window.location.href = "/tokyo"; // Redirect to /tokyo
    }
});
